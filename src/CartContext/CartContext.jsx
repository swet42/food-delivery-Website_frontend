...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get('https://swet42-food-delivery-website-backen.vercel.app/api/cart', {
  ...
  const addToCart = useCallback(async (item, qty) => {
    ...
    const res = await axios.post(
      'https://swet42-food-delivery-website-backen.vercel.app/api/cart',
  ...
  const updateQuantity = useCallback(async (_id, qty) => {
    ...
    const res = await axios.put(
      `https://swet42-food-delivery-website-backen.vercel.app/api/cart/${_id}`,
      { quantity: qty },
      { withCredentials: true, headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  const clearCart = useCallback(async () => {
    const token = localStorage.getItem('authToken');
    await axios.post(
      'https://swet42-food-delivery-website-backen.vercel.app/api/cart/clear',
      {},
      { withCredentials: true, headers: { Authorization: `Bearer ${token}` } }
    );
